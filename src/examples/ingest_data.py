...
async def load_pdf_data(file_names: list[str]) -> list[Document]:
    logger.info(f"STEP 1. Loading PDF data from {data_directory} ...")
    # create a list to store all pages from all PDF documents
    docs_pages = []
    # load pages from the PDF documents
    for i, file_name in enumerate(file_names, start=1):
        if file_name.lower().endswith(".pdf"):
            logger.info(f"  [{i}/{len(file_names)}] Loading {file_name} ...")
            loader = PyPDFLoader(
            ...
            logger.info(f"  Loaded {len(docs_pages)} pages from {file_name}")
    logger.info(
...
def filter_metadata(docs_pages: list[Document]) -> list[Document]:
    logger.info("STEP 1.1. Filtering metadata from the loaded documents ...")
    ...
    logger.info(f"  Filtered metadata from {len(docs_pages)} documents")

    return docs_pages


def split_data(docs_pages: list[Document]) -> list[Document]:
    logger.info("STEP 2. Splitting the loaded documents into chunks ...")
    ...
    logger.info(f"  Splitted {len(docs_pages)} documents into {len(doc_splits)} chunks")
...
def embed_and_store_data(doc_splits: list[Document], file_names: list[str]):
    ...
    logger.info(f"  Indexed {len(chunk_indexes)} chunks in the vector store")
